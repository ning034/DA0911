...
{#1.一般参数模型####
  ...
  write.csv(output1,file = "D:/D/data/xuhuibusiness/output1_1014.csv",row.names = F) #输出为table
}
...
{##2.品类模型####
  ...
  retailtypename 
    ...
    write.csv(output2,file = "D:/D/data/xuhuibusiness/output2_1014.csv",row.names = F) #输出为table
}
...
{#3.品类特征模型####
  ...
  summary(cat$amt)
  w <- which(cat$amt <= 16000000 & cat$amt > 3709005 )
  cat$size <- "large" 
  cat[w,]$size <- "medium"
  ...
  for(i in j){
    result1 <- data.frame(cat=retailtypename[i],acv=zz$Estimate[i],t_value=zz$`t value`[i],acv2=zz$Estimate[i+1],t_value2=zz$`t value`[i+1],star1=zz$`   `[i],star2=zz$`   `[i+1],row.names = NULL)
    result <- rbind(result,result1)
  }
  result
  result$acv <-round(result$acv,5)
  ...
  write.csv(output3,file = "D:/D/data/xuhuibusiness/output3_1014.csv",row.names = F) #输出为table
